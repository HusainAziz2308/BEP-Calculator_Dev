const deadactivateTabs = function(){
  $$("#tab").forEach((tab, index) => {
    tab.className = "px-5 border-b-4 border-white focus:outline-none text-green-700 z-10";
    $$("#tab-content")[index].className = "w-full h-screen flex-col sm:flex-row hidden";
  })
}

const activateTab = function(index){
  $$("#tab")[index].className = "px-5 border-b-4 border-green-600 focus:outline-none text-blue-700 z-10";
